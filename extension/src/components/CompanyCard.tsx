...
/* ─── Save dropdown ──────────────────────────────────────────────────────── */
function SaveDropdown({ companyId, lists, onSaved }: { companyId: string; lists: LeadsList[]; onSaved?: () => void }) {
  ...
  const save = async (list: LeadsList) => {
    ...
    setSaving(list.id);
    try {
      await listsApi.addItems(list.id, [{ record_id: companyId, item_type: 'company' }]);
      setSaved(list.id);
      onSaved?.();
    } catch { /* ignore */ }
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {open && !saved && lists.length > 0 && (
        <div style={{ position: 'absolute', bottom: 'calc(100% + 6px)', right: 0, zIndex: 50, background: '#fff', border: '1px solid #E5E7EB', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', minWidth: 180, overflow: 'hidden' }}>
          <p style={{ padding: '8px 12px 6px', fontSize: 10, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0, borderBottom: '1px solid #F3F4F6' }}>Save to list</p>
          {lists.map(list => (
            <button key={list.id} onClick={() => save(list)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'none', border: 'none', cursor: saving ? 'default' : 'pointer', fontSize: 12.5, color: '#111827', textAlign: 'left', transition: 'background 0.1s' }}
...
/* ─── Main component ─────────────────────────────────────────────────────── */
export function CompanyCard({ company, lists = [], onRefreshLists }: Props) {
        ...
        <div style={{ borderTop: '1px solid #F1F5F9', padding: '10px 18px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {lists.length > 0 && (
            <div style={{ marginLeft: 'auto' }}>
              <SaveDropdown companyId={company.id} lists={lists} onSaved={onRefreshLists} />
            </div>
          )}
        </div>

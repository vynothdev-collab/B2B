...
export function Avatar({
  ...
  return (
    <div
      className={`${dim} shrink-0 flex items-center justify-center rounded-full font-semibold text-white ${avatarColor(name)}`}
    >
      {initials(name)}
    </div>
  );
...
export function CompanyLogo({
  ...
  return (
    <div
      className={`${dim} shrink-0 flex items-center justify-center rounded ${textSize} font-bold text-white ${avatarColor(name)}`}
    >
      {name[0]?.toUpperCase()}
    </div>
  );
...
export function normalizeSizeRange(raw: string): string {
  const cleaned = raw
    .replace(/\s*employees?/gi, "")
    .replace(/,/g, "")
    .trim();
  const lc = cleaned.toLowerCase();
  if (lc === "myself only" || lc === "self employed" || lc === "1")
    return "Solo";
  if (lc === "0" || lc === "") return "—";

...
import CompanyFilterPanel, {
  countCompanyFilters,
} from "./filters/CompanyFilterPanel";
...
export default function CompanySearchPage() {
  ...
  const {
    ...
    reset,
    cols,
  } = useColumnSettings("b2b:col:companies", COMPANY_COLUMNS);
  ...
  const selectedCompanies = results
    ? ((results.data ?? []) as CompanyResult[]).filter((r) =>
        selected.has(r.id),
      )
    : [];
    ...
    <>
      ...
      <div className="flex min-w-0 flex-1 gap-2 overflow-hidden px-2 py-2 sm:px-3">
        <FilterPanelShell
          ...
          loading={loading}
          totalCount={countCompanyFilters(filters)}
        >
          <CompanyFilterPanel
            filters={filters}

...
function KeywordInput({
  ...
  const showDropdown =
    withAutocomplete && open && (loading || suggestions.length > 0);
      ...
      {values.length < MAX_KEYWORDS && (
        <div ref={containerRef} className="relative">
          <input
            ...
            onKeyDown={handleKey}
            onBlur={() => {
              if (!withAutocomplete) commit(draft);
            }}
    ...
    <div className="space-y-3">
      <div>
        <div className="mb-1 flex items-center justify-between">
          ...
          <span className="text-[10px] text-gray-400">
            {include.length}/{MAX_KEYWORDS} — min {MIN_CHARS} characters
          </span>
        </div>
        <KeywordInput
          values={include}
      ...
      <div className="rounded-lg border border-gray-200 overflow-hidden">
        ...
        >
          ...
          <span
            className={`relative inline-flex h-4 w-7 shrink-0 rounded-full transition-colors ${searchEverywhere ? "bg-red-600" : "bg-gray-200"}`}
          >
            <span
              className={`absolute top-0.5 h-3 w-3 rounded-full bg-white shadow-sm transition-transform ${searchEverywhere ? "translate-x-3.5" : "translate-x-0.5"}`}
            />
          </span>
        </button>
        ...
        <div className="border-t border-gray-100 px-3 pb-1.5 pt-1 space-y-0.5">
          {SCOPES.map((s) => {
            ...
            return (
              ...
              >
                <span
                  className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border transition-colors ${checked ? "border-red-500 bg-red-500" : "border-gray-300 bg-white"}`}
                >
                  {checked && (
                    <svg
      ...
      <div>
        <div className="mb-1 flex items-center justify-between">
          <span
            className={`text-[12px] font-medium ${include.length === 0 ? "text-gray-300" : "text-red-600"}`}
          >
            Exclude keywords
          </span>
          {include.length === 0 && (
        ...
        {include.length === 0 ? (
          <>
            ...
            <p className="mt-1 text-[10px] text-gray-400">
              Add at least one include keyword to enable excludes.
            </p>
          </>
        ) : (
          <KeywordInput

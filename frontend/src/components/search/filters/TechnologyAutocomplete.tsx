...
import { fetchTechnologySuggestions } from "@/lib/searchApi";
...
export default function TechnologyAutocomplete({ label, placeholder, values, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      const results = await fetchTechnologySuggestions(text);
  ...
  const showDropdown = open && (loading || suggestions.length > 0 || text.trim().length >= 1);

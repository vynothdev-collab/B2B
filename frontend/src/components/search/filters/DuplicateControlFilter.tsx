...
const labelCls =
  "mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-gray-400";
const inputCls =
...
function Checkbox({
  ...
  return (
    ...
    >
      <span
        className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors ${checked ? "border-red-500 bg-red-500" : "border-gray-300 bg-white"}`}
      >
        {checked && (
          <svg
      ...
      <span className={`flex-1 text-[12px] ${checked ? "font-medium" : ""}`}>
        {label}
      </span>
    </button>
  );
    ...
    <div className="mt-1 ml-6 flex flex-col gap-0.5">
      {lists.map((lst) => {
        ...
        return (
          ...
          >
            <span
              className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border ${sel ? "border-red-500 bg-red-500" : "border-gray-300 bg-white"}`}
            >
              {sel && (
                <svg
...
export default function DuplicateControlFilter({ filters, onChange }: Props) {
  ...
  const addExclusion = () => {
    ...
    if (v && !filters.exclusionCompanyNames.includes(v)) {
      onChange({
        exclusionCompanyNames: [...filters.exclusionCompanyNames, v],
      });
    }
    setDraft("");
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <Checkbox
          checked={false}
          label="Hide people already exported"
          onChange={() => {}}
          disabled
        />
      ...
      <div>
                  ...
                  onClick={() =>
                    onChange({
                      exclusionCompanyNames:
                        filters.exclusionCompanyNames.filter((x) => x !== v),
                    })
                  }
        ...
        <input
          ...
          className={inputCls}
        />
        <p className="mt-0.5 px-0.5 text-[10px] text-gray-400">
          Press Enter to add. Excludes people at matching companies.
        </p>
      </div>
    </div>
  );

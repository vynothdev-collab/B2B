...
function buildCompanyColumns({
  visibleColumns,
  onNameClick,
}: BuildColsArgs): DataTableColumn<CompanyResult>[] {
  const rawCols: DataTableColumn<CompanyResult>[] = [
    ...
    },
    {
      ...
      render: (company) => {
        const statusBadgeClass =
          STATUS_COLORS[(company.company_status ?? "").toLowerCase()] ??
    ...
    },
    {
      ...
      minWidth: 160,
      render: (company) => (
        <ChipList items={toStringArr(company.categories_and_keywords)} />
      ),
    },
    {
    ...
    },
    {
      ...
      minWidth: 155,
      render: (company) => (
        <ChipList items={toStringArr(company.awards_certifications)} />
      ),
    },
    {
      key: "growth",
    ...
    },
    {
      ...
      minWidth: 90,
      render: (company) =>
        company.total_website_visits_change?.change_yearly_percentage !=
        null ? (
          <span className="text-[13px] font-medium text-gray-800">
            {company.total_website_visits_change.change_yearly_percentage >= 0
              ? "+"
              : ""}
            {company.total_website_visits_change.change_yearly_percentage.toFixed(
              1,
            )}
            %
          </span>
    ...
    },
    {
      ...
      render: (company) => {
        ...
        if (!r) return <Dash />;
        const src =
          r["source_4_annual_revenue_range"] ??
          r["source_6_annual_revenue_range"];
        if (!src) return <Dash />;
    ...
    },
    {
      ...
      render: (company) =>
        company.active_job_postings != null ? (
          <span className="text-[13px] text-gray-800">
            {Array.isArray(company.active_job_postings)
              ? company.active_job_postings.length
              : "—"}
          </span>
        ) : (
          <Dash />
    ...
    },
    {
      ...
      render: (company) => (
        <ChipList
          items={(company.technologies_used ?? [])

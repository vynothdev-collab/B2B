...
import { connectInstantly } from "@/lib/instantlyApi";
...
export default function ConnectInstantlyModal({ open, onClose, onConnected }: Props) {
  ...
  async function handleConnect() {
    ...
    setError(null);
    try {
      await connectInstantly(apiKey.trim());
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-1.5rem)] max-w-[420px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3.5">
          <h2 className="text-sm font-semibold text-gray-900">Connect Instantly</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-4 py-4">
          <p className="text-xs leading-snug text-gray-500">
            Paste an API key from Instantly → Settings → Integrations → API.
            Your key is encrypted at rest and used only to push leads on your
            behalf.
          </p>

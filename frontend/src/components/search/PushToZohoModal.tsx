...
import { pushToZoho, type ZohoPushItemResult } from "@/lib/zohoApi";
...
export default function PushToZohoModal({ open, onClose, items }: Props) {
  ...
  async function handlePush() {
    ...
    try {
      ...
      if (res.pushed > 0 && res.failed === 0) {
        ...
        toast.success(`Pushed ${res.pushed} ${noun}${res.pushed !== 1 ? "s" : ""} to Zoho CRM.`);
      }
    } catch (e) {
      toast.apiError(e);
    } finally {
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-1.5rem)] max-w-[420px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3.5">
          <h2 className="text-sm font-semibold text-gray-900">Push to Zoho CRM</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          {!result ? (
            <>
              <p className="text-xs leading-snug text-gray-500">
                ...
                {itemType === "company"
                  ? "Records will be pushed as Zoho Accounts."
                  : "Records will be pushed as Zoho Leads with whatever contact info is unlocked."}
              </p>
              <button
                type="button"
                onClick={handlePush}

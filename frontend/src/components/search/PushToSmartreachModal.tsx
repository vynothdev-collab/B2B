...
import {
  getSmartreachCampaigns,
  pushToSmartreach,
  type SmartreachCampaign,
  type SmartreachPushItemResult,
...
export default function PushToSmartreachModal({ open, onClose, items, onPushed }: Props) {
  const [campaigns, setCampaigns] = useState<SmartreachCampaign[]>([]);
  ...
  useEffect(() => {
    ...
    getSmartreachCampaigns()
      ...
      .catch(() => toast.error("Failed to load Smartreach campaigns."))
  ...
  async function handlePush() {
    ...
    setPushing(true);
    try {
      const res = await pushToSmartreach(
        items.map((i) => ({ record_id: i.record_id, item_type: i.item_type, data: {} })),
        campaignId,
      );
      setResult(res);
      if (res.pushed > 0 && res.failed === 0) {
        toast.success(`Pushed ${res.pushed} prospect${res.pushed !== 1 ? "s" : ""} to Smartreach.`);
      }
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-1.5rem)] max-w-[420px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3.5">
          <h2 className="text-sm font-semibold text-gray-900">Push to Smartreach</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          {!result ? (
            <>
              <p className="text-xs leading-snug text-gray-500">
                ...
                records with an unlocked work email will be added as
                Smartreach prospects.
              </p>
              ...
              >
                ...
                {pushing ? "Pushing…" : "Push to Smartreach"}
              </button>
            </>
          ) : (
            <>

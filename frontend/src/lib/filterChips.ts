...
export function buildPersonChips(
  ...
  filters.name.forEach((v) =>
    chips.push({
      id: `name-${v}`,
      label: v,
      onRemove: () => onChange({ name: filters.name.filter((x) => x !== v) }),
    }),
  );
  ...
  filters.jobTitle.forEach((v) =>
    chips.push({
      ...
      label: v,
      onRemove: () =>
        onChange({ jobTitle: filters.jobTitle.filter((x) => x !== v) }),
    }),
  );
  ...
  filters.departments.forEach((v) =>
    chips.push({
      id: `dept-${v}`,
      label: labelOf(DEPARTMENT_OPTIONS, v),
      onRemove: () =>
        onChange({ departments: filters.departments.filter((x) => x !== v) }),
    }),
  );
  ...
  filters.seniority.forEach((v) =>
    chips.push({
      id: `sen-${v}`,
      label: labelOf(SENIORITY_OPTIONS, v),
      onRemove: () =>
        onChange({ seniority: filters.seniority.filter((x) => x !== v) }),
    }),
  );
  ...
  filters.personLocationCountries.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          personLocationCountries: filters.personLocationCountries.filter(
            (x) => x !== v,
          ),
        }),
    }),
  ...
  filters.personLocationStates.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          personLocationStates: filters.personLocationStates.filter(
            (x) => x !== v,
          ),
        }),
    }),
  ...
  filters.personLocationCities.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          personLocationCities: filters.personLocationCities.filter(
            (x) => x !== v,
          ),
        }),
    }),
  ...
  filters.companyHQCountries.forEach((v) =>
    chips.push({
      id: `hq-country-${v}`,
      label: `HQ: ${capitalize(v)}`,
      onRemove: () =>
        onChange({
          companyHQCountries: filters.companyHQCountries.filter((x) => x !== v),
        }),
    }),
  );
  filters.companyHQStates.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          companyHQStates: filters.companyHQStates.filter((x) => x !== v),
        }),
    }),
  );
  filters.companyHQCities.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          companyHQCities: filters.companyHQCities.filter((x) => x !== v),
        }),
    }),
  );
  ...
  if (filters.requireWorkEmail)
    chips.push({
      id: "email",
      label: "Work Email",
      onRemove: () => onChange({ requireWorkEmail: false }),
    });

  filters.companyStatus.forEach((v) =>
    chips.push({
      id: `status-${v}`,
      label: labelOf(COMPANY_STATUS_OPTIONS, v),
      onRemove: () =>
        onChange({
  ...
  filters.companyType.forEach((v) =>
    chips.push({
      id: `type-${v}`,
      label: labelOf(INDUSTRY_OPTIONS, v),
      onRemove: () =>
        onChange({ companyType: filters.companyType.filter((x) => x !== v) }),
    }),
  );
  ...
  filters.revenueBuckets.forEach((v) =>
    chips.push({
      id: `rev-${v}`,
      label: labelOf(REVENUE_OPTIONS, v),
      onRemove: () =>
        onChange({
  ...
  if (filters.fundingMode === "predefined" && filters.fundingPresets.length)
    chips.push({
      id: "funding",
      label: `Funding: ${presetLabels(filters.fundingPresets, FUNDING_PRESETS)}`,
      onRemove: () => onChange({ fundingPresets: [] }),
    });
  else if (filters.fundingMode === "custom") {
    const fundRange = rangeLabel(
      filters.fundingMin,
      filters.fundingMax,
      fmtUsd,
    );
    if (fundRange)
      chips.push({
        id: "funding",
        label: `Funding: ${fundRange}`,
        onRemove: () => onChange({ fundingMin: "", fundingMax: "" }),
      });
  }

  if (
    filters.headcountGrowthMode === "predefined" &&
    filters.headcountGrowthPresets.length
  )
    chips.push({
      id: "growth",
      label: `Growth: ${presetLabels(filters.headcountGrowthPresets, GROWTH_PRESETS)}`,
      onRemove: () => onChange({ headcountGrowthPresets: [] }),
    });
  else if (filters.headcountGrowthMode === "custom") {
    const growthRange = rangeLabel(
  ...
  if (filters.foundedMode === "predefined" && filters.foundedPresets.length)
    chips.push({
      id: "founded",
      label: `Founded: ${presetLabels(filters.foundedPresets, FOUNDED_YEAR_PRESETS)}`,
      onRemove: () => onChange({ foundedPresets: [] }),
    });
  ...
  if (roleMin || roleMax)
    chips.push({
      id: "time-role",
      label: `Time in Role: ${roleMin ?? 0}–${roleMax ?? "∞"} mo`,
      onRemove: () =>
        onChange({
          timeInRoleMinYears: "",
          timeInRoleMinMonths: "",
  ...
  if (coMin || coMax)
    chips.push({
      id: "time-company",
      label: `Time in Company: ${coMin ?? 0}–${coMax ?? "∞"} mo`,
      onRemove: () =>
        onChange({
  ...
  if (expRange)
    chips.push({
      id: "experience",
      label: `Experience: ${expRange}`,
      onRemove: () =>
        onChange({ experienceYearsMin: "", experienceYearsMax: "" }),
    });

  if (filters.jobChangeTimeframe) {
    const label =
      JOB_CHANGE_TIMEFRAMES.find((o) => o.value === filters.jobChangeTimeframe)
        ?.label ?? `Last ${filters.jobChangeTimeframe}m`;
    chips.push({
      id: "job-change",
      label: `Job Change: ${label}`,
      onRemove: () => onChange({ jobChangeTimeframe: "" }),
    });
  }
  ...
  if (filters.hideAllSavedPeople)
    chips.push({
      id: "dup-people",
      label: "Hiding saved people",
      onRemove: () =>
        onChange({ hideAllSavedPeople: false, hideSavedPeopleListIds: [] }),
    });

  if (filters.hideAllSavedCompanies)
    chips.push({
      id: "dup-companies",
      label: "Hiding saved companies",
      onRemove: () =>
        onChange({ hideAllSavedCompanies: false, hideSavedCompanyListIds: [] }),
    });
  ...
  filters.exclusionCompanyNames.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          exclusionCompanyNames: filters.exclusionCompanyNames.filter(
            (x) => x !== v,
          ),
        }),
    }),
  ...
  filters.certifications.forEach((v) =>
    chips.push({
      id: `cert-${v}`,
      label: labelOf(CERTIFICATION_OPTIONS, v),
      onRemove: () =>
        onChange({
          certifications: filters.certifications.filter((x) => x !== v),
        }),
    }),
  );
  ...
  filters.otherCompliance.forEach((v) =>
    chips.push({
      ...
      onRemove: () =>
        onChange({
          otherCompliance: filters.otherCompliance.filter((x) => x !== v),
        }),
    }),
  );
...
export function buildCompanyChips(
  ...
  filters.companyHowTheySell.forEach((v) =>
    chips.push({
      id: `sell-${v}`,
      label: labelOf(COMPANY_HOW_THEY_SELL_OPTIONS, v),
      onRemove: () =>
        onChange({
          companyHowTheySell: filters.companyHowTheySell.filter((x) => x !== v),
        }),
    }),
  );
  ...
  filters.companyMoreFlags.forEach((v) =>
    chips.push({
      id: `flag-${v}`,
      label: labelOf(COMPANY_MORE_FLAGS_OPTIONS, v),
      onRemove: () =>
        onChange({
          companyMoreFlags: filters.companyMoreFlags.filter((x) => x !== v),
        }),
    }),
  );
  ...
  filters.companyRevenueModel.forEach((v) =>
    chips.push({
      id: `rev-model-${v}`,
      label: labelOf(COMPANY_REVENUE_MODEL_OPTIONS, v),
      onRemove: () =>
        onChange({
          companyRevenueModel: filters.companyRevenueModel.filter(
            (x) => x !== v,
          ),
        }),
    }),
  ...
  filters.emailProviders.forEach((v) =>
    chips.push({
      id: `email-${v}`,
      label: `Email: ${labelOf(EMAIL_PROVIDER_OPTIONS, v)}`,
      onRemove: () =>
        onChange({
          emailProviders: filters.emailProviders.filter((x) => x !== v),
        }),
    }),
  );
  ...
  filters.awards.forEach((v) =>
    chips.push({
      ...
      label: `Award: ${v}`,
      onRemove: () =>
        onChange({ awards: filters.awards.filter((x) => x !== v) }),
    }),
  );
  ...
  if (visitsRange)
    chips.push({
      id: "visits",
      label: `Monthly Visits: ${visitsRange}`,
      onRemove: () => onChange({ websiteVisitsMin: "", websiteVisitsMax: "" }),
    });

  const visitChangeRange = rangeLabel(
  ...
  if (visitChangeRange)
    chips.push({
      id: "visit-change",
      label: `Visit Change (${filters.visitChangeTimeframe}): ${visitChangeRange}`,
      onRemove: () => onChange({ visitChangeMin: "", visitChangeMax: "" }),
    });

  if (filters.trafficCountry) {
    const countryRange = rangeLabel(
      filters.trafficCountryMin,
      filters.trafficCountryMax,
      (n) => `${n}%`,
    );
    chips.push({
      id: "traffic-country",
  ...
  filters.companyNewsCategories.forEach((v) =>
    chips.push({
      id: `news-cat-${v}`,
      label: labelOf(COMPANY_NEWS_CATEGORIES, v),
      onRemove: () =>
        onChange({
  ...
  if (filters.companyNewsTimeframe) {
    const tf = COMPANY_NEWS_TIMEFRAMES.find(
      (o) => o.value === filters.companyNewsTimeframe,
    );
    chips.push({
      id: "news-tf",
      label: `News: ${tf?.label ?? filters.companyNewsTimeframe}`,
      onRemove: () => onChange({ companyNewsTimeframe: "" }),
    });
  }

  const empRange = rangeLabel(
  ...
  filters.fundingStages.forEach((v) =>
    chips.push({
      id: `stage-${v}`,
      label: labelOf(FUNDING_STAGE_OPTIONS, v),
      onRemove: () =>
        onChange({
          fundingStages: filters.fundingStages.filter((x) => x !== v),
        }),
    }),
  );

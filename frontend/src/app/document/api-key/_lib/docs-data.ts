...
export const WEBHOOK_FIELD_MAPPING: WebhookFieldMapping[] = [
  { leadsbuddyField: "Unlocked work email", payloadField: "record.email", notes: "Only sent if the record's work email has been unlocked. A push is rejected if no work email is unlocked." },
  { leadsbuddyField: "first_name / last_name", payloadField: "record.first_name / record.last_name", notes: "last_name falls back to the person's full name if not known." },
  { leadsbuddyField: "Unlocked mobile number", payloadField: "record.phone", notes: "Only sent if the mobile number has been unlocked." },
  { leadsbuddyField: "active_experience_title", payloadField: "record.job_title", notes: "Current job title, if known." },
  { leadsbuddyField: "active_experience_company_name / _website", payloadField: "record.company_name / record.company_website", notes: "Current employer, if known." },
  { leadsbuddyField: "location_city / location_state / location_country", payloadField: "record.city / record.state / record.country", notes: "Person's location, if known." },
  { leadsbuddyField: "linkedin_url", payloadField: "record.linkedin_url", notes: "Person's LinkedIn profile, if known." },
];

export const SAMPLE_WEBHOOK_CONNECT_RESPONSE = `{
  "connected": true,
  "webhook_url": "https://hooks.zapier.com/hooks/catch/12345/abcde/",
  "signing_secret": "9f2b6e2a1234...redacted-shown-once",
  "connected_at": "2026-08-01T09:12:00Z"
}`;
...
export const SAMPLE_WEBHOOK_PAYLOAD = `{
  ...
  "pushed_at": "2026-08-17T14:02:11Z"
}`;

export const SAMPLE_WEBHOOK_PUSH_RESPONSE = `{
  "pushed": 2,
  "failed": 1,
  "results": [
    { "record_id": "abc123", "delivered": true, "error": null },
    { "record_id": "def456", "delivered": true, "error": null },
    { "record_id": "ghi789", "delivered": false, "error": "Work email must be unlocked before pushing." }
  ]
}`;
